//ARCHIVO CORRESPONDIENTE A LA PARTE 2. CREACIÓN DEL DEADLOCK
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
#include <vector>
...
void* DasneyContenido(void* arg) {
    while (semanas > 0) {
        ...
        cout<<"esperando que count de dasney sea 6"<<endl;
...
void* Dasney(void* arg) {
    ...
    while (semanas>0)
    {
        ...
        count++;
         if(count == 6) {
            pthread_cond_signal(&cond_actualizarDasney);
         }
        pthread_mutex_unlock(&mutexCount);
        if(count==6){
             NuevaSemana(nullptr); 
        }
        cout<<"profesor "<<thread_index<<" esperando actualizacion para acceder a la plataforma"<<endl;
